...
- Validate untrusted input at the boundary. Check path traversal, injection, unsafe deserialization, SSRF, open redirects, unsafe file handling, and untrusted origins.
- Treat frontend configuration as public. Never place secrets, signing keys, private URLs, or credentials in browser-delivered code.
- Keep secrets in environment or a secret manager. Do not log tokens, passwords, raw request bodies, or unnecessary personal data.
